...
This class contains all the information of the data provided by CPO.
...
NOTE: This is not a standard field, it is only available when the CDR is created by specific CPO.


[[mod_cdrs_cst_custom_data_type_enum]]
==== CustomDataType _enum_

This enumeration contains allowed values for CustomDataType, which are used to define the type of the data provided by CPO.

[cols="3,10",options="header"]
|===
|Value |Description
|Receipt | Charging session receipt issued by CPO
...
This enumeration contains allowed values for CustomDataFormat, which are used to define the format of the data from "data" field provided by CPO.
