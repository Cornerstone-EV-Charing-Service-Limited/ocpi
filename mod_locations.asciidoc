...
|ocpp_status |<<mod_locations_ocpp_status_enum,Status>> |1 |Indicates the current status of the EVSE in the presentation of OCPP 1.6. 
...
[[mod_locations_ocpp_status_enum]]
==== OCPPStatus _enum_

The status of an EVSE in presentation of OCPP 1.6.

[cols="3,10",options="header"]
|===
|Value |Description 

|AVAILABLE |The connector is free and ready for a new EV to plug in and start a charging session.  
|PREPARING |A vehicle is plugged in, and the system is performing the initial steps, such as communication and authorization, before charging begins. 
|CHARGING |The charger is actively delivering power to the EV.  
|SUSPENDEDEVSE |The charger has ended the charging session for a reason related to the charger itself, such as load management or power settings. 
|SUSPENDEDEV |The EV has stopped receiving power, perhaps because its battery is full or the vehicle has been commanded to stop. 
|FINISHING |The charging session has concluded, but the vehicle remains plugged into the port. 
|RESERVED |The port is held for a specific user or vehicle and will not accept other users. 
|FAULTED |The EVSE/Connector was discontinued/removed. 
|UNAVAILABLE | The charger is being set to inoperative due to lack of power or under maintenance.
|OFFLINE| The server didn't receive any OCPP message from charger and consider it is network disconnected.
|===
